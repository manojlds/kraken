...
resource "template_file" "node_cloudinit_special" {
  ...
  vars {
    ...
    format_rkt_storage_mnt = "${lookup(var.format_rkt_storage_mnt, element(split(",", var.aws_storage_type_special_rkt), count.index))}"
    coreos_update_channel = "${var.coreos_update_channel}"
    coreos_reboot_strategy = "${var.coreos_reboot_strategy}"
...
resource "template_file" "node_cloudinit" {
  ...
  vars {
    ...
    format_rkt_storage_mnt = "${lookup(var.format_rkt_storage_mnt, var.aws_storage_type_node_rkt)}"
    coreos_update_channel = "${var.coreos_update_channel}"
    coreos_reboot_strategy = "${var.coreos_reboot_strategy}"
